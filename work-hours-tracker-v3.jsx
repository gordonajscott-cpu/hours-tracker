...
export default function WorkHoursTracker() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Restore timer state
        try {
          const tr = await window.storage.get(TIMER_KEY).catch(() => null);
          if (tr?.value) {
        ...
        // Load tasks
        try {
          const tk = await window.storage.get(TASKS_KEY).catch(() => null);
  ...
  const save = useCallback(async (data, cfg, hrs, defs) => {
    try {
      ...
      await Promise.all([
        window.storage.set(DATA_KEY, JSON.stringify(data)),
        window.storage.set(CONFIG_KEY, JSON.stringify(cfg)),
        window.storage.set(SETTINGS_KEY, JSON.stringify({ standardHours: hrs, defaults: defs }))
      ]);
      setLastSaved(new Date());
      setSaveStatus("saved");
  ...
  useEffect(() => {
    ...
    window.storage.set(TIMER_KEY, JSON.stringify(timerState)).catch(() => {});
  ...
  useEffect(() => {
    if (loading) return;
    const t = setTimeout(() => {
      window.storage.set(TASKS_KEY, JSON.stringify(tasks)).catch(() => {});
    }, 600);
    return () => clearTimeout(t);
  }, [tasks, loading]);
